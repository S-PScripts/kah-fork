local Settings = {
    ["Autorun Commands"] = {"antikick","antiabuse","antiname","localremoveobby","fixvelocity","runcommand logs","antivoid"}, -- Commands to run automatically
    ["Default Whitelisted"] = {}, -- People whitelisted by default
    ["Default Banned"] = {"794x794","Instlgator","blue20043"}, -- People banned by default
...
function runCommand(param1,specargs)
    for i,asdfuhiswuejfniuserf in ipairs(commandlist) do
...
addCommand("potatocrash",{"player"},function(args)
    ...
    wait(0.1)
    if cacheAntiKick then runCommand(".antikick",{}) end
end)

addCommand("swordcrash",{"player"},function(args)
    local cacheAntiKick = Loops.antikick
    if cacheAntiKick then Loops.antikick = false end
    local Player = GetPlayers(args[1])
    for i,v in pairs(Player) do
...
TextLabel_2.Text = "[ii_SaucyyRon]: deez nuts"
...
addCommand("loopgrab",{},function()
    runCommand(".autoadmin",{})
end)

addCommand("unloopgrab",{},function()
    runCommand(".unautoadmin",{})
end)
    ...
    repeat game:GetService("RunService").RenderStepped:Wait()
        ...
        for i,v in pairs(game:GetService("Workspace").Terrain["_Game"].Admin.Pads:GetChildren()) do
            if table.find(PadNames,v.Name) then
                ...
                spawn(function()
                    wait(0.3)
                    GUI:SendMessage("ii' s St upid Ad min", "Please only grab a singular admin pad, "..game.Players[v.Name:split("'s")[1]].DisplayName..".")
                end)
            else
                if v.Name ~= "Touch to get admin" then
...
wait(0.25)
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame=pos
if antiNameCache then spawn(function() runcommand(".antiname",{}) end) end
end)
...
addCommand("stopscript",{},function()
    spawn(function()
        if game.Chat:FindFirstChild("Obby") then runCommand(".localaddobby",{}) end
    end)
...
addCommand("fixcolor",{},function()
    ...
    spawn(function()
    if game.Chat:FindFirstChild("Obby") then ObbyDestroyed = true runCommand(".localaddobby",{}) end
    end)
    spawn(function()
...
wait(1.5)
			if ObbyDestroyed then runCommand(".localremoveobby",{}) end
end)
    ...
    repeat game:GetService("RunService").RenderStepped:Wait()pcall(function()
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character.HumanoidRootPart then
            if game.Players.LocalPlayer.Character.HumanoidRootPart.Position.Y < -7 then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(game.Players.LocalPlayer.Character.HumanoidRootPart.Position.X,0,game.Players.LocalPlayer.Character.HumanoidRootPart.Position.Z)
...
addCommand("racheliscool",{"player"},function(args)
    runCommand(".antiserverlock",args)
end)
...
addCommand("unracheliscool",{"player"},function(args)
    runCommand(".unantiserverlock",args)
end)
...
local function PlayerAdded(Player)
    spawn(function()
        ...
        if table.find(Banned,Player.Name) then
            ...
            wait(0.25)
            runCommand(".potatocrash",{Player.Name})
        end
        ...
        if ServerLocked and not table.find(Whitelisted,Player.Name) and not table.find(ServerLockedProtection,Player.Name) then
                if ServerLockedSoundEnabled then
                    ...
                    repeat wait() until Player and Player.Character
                    wait(0.25)
                    runCommand(".potatocrashsound",{Player.Name,ServerLockedSound})
                else
                    spawn(function()
                        GUI:SendMessage("ii' s St upid Ad min", "This server is currently locked.")
                    end)
