...
local hasNewVersion = httpService:GetAsync("https://krypton.sergioesquina.repl.co/roblox/exploit/script/KohlsCool/latestversion")
local update = nil
local updateMod = nil
if tonumber(version) < tonumber(hasNewVersion) then update = true end
if tonumber(version) > tonumber(hasNewVersion) then updateMod = true end
...
if update ~= nil then alert("None", "Hey! There is an update, you are using v"..version.." and the latest is v"..hasNewVersion) end
if updateMod ~= nil then alert("None", "Hello Mod, you are using a unreleased version that may have bugs (v"..version..")") end
...
if player.name == b then
  ...
  createcmd(player, msg, prefix, "report", function()
    ...
    local reason = string.gsub(arg[1], '"', "")
    local target = string.split(arg[0], " ")[1]
