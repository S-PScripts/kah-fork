...
local version_ = "jbwAez-A3uXLK-xddJX2"
...
local whitelisted = { }
local blacklisted = {2786458053,2840154843,1204159402,2840319606,1541950191}
...
spawn(function()
	...
	while true do
		local color = Tween:Create(colorPart, TweenInfo.new(.5), {Color = Color3.fromRGB(math.random(0,255),math.random(0,255),math.random(0,255))})
		color:Play()
		wait(.5)
	end
end)
...
local function ConnectP(plr)
	...
	if IsBlacklisted then
		msg("anti crash enabled so dont complain")
		local anti = RunService.RenderStepped:Connect(function()
			if plr.UserId == 1541950191 then
				chat("ungear all")
			else
				chat("size all 10")
			end
...
commands.crash = function()
	...
	PlrChar:WaitForChild("VampireVanquisher")
	while wait() do
