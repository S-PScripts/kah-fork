...
local blacklisted = {
	...
	nonperm = {2786458053,2840154843,2840319606,1541950191,2382153210,2835013669,2579306089,2789215822,2279597178,964474189,2899767220,2276224205,2385681508,2992513916},
...
commands.troubleshoot = function()
	local c364193 = Color3.fromRGB(198, 132, 0)
	local arr23f2q = #Game_.Admin.Pads:GetChildren()
	if arr23f2q == 0 then
		notif("troubleshoot: all admin pads missing",c364193)
	elseif arr23f2q == 9 then
		notif("troubleshoot: all admin pads found",c364193)
	else
		notif("troubleshoot: "..tostring(9-arr23f2q).." admin pads missing",c364193)
	end
...
commands.cmds = function()
	...
	print("troubleshoot -- yes u will see things")
	print("antigc [on/off] -- prevents you from being color blinded by the time stop gear")
